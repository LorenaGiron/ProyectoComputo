            ...
            <tbody>
              {zonas.map((z, i) => (
                <tr key={i} className="border-b border-lila/5 hover:bg-lila/5 transition-colors">
                  <td className="px-6 py-4 text-blanco font-medium">{z.zona}</td>
                  <td className="px-6 py-4 text-lila-soft">{z.tiempo}</td>
                  <td className="px-6 py-4 text-lila-soft">{z.costo}</td>
                  <td className="px-6 py-4 text-verde font-semibold">{z.gratis}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={card.titulo} className="bg-oscuro-card border border-lila/10 rounded-2xl p-7">
            ...
            <h3 className="text-blanco font-semibold text-lg mb-3">{card.titulo}</h3>
            <p className="text-base text-lila-soft leading-relaxed">{card.texto}</p>
          </div>
        ))}
      </div>
